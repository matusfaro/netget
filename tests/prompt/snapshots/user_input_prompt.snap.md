...
- **Complex authentication logic** with well-defined conditions (e.g., SSH auth with specific username/password
  combinations)
...
- `script_runtime`: "Python, JavaScript, Go, or Perl" (REQUIRED when script_inline is provided - choose the appropriate
  runtime)
- `script_inline`: Your script code as a string (when provided, script_runtime MUST also be specified)
- `script_handles`: Array of event types to handle (e.g., `["ssh_auth", "ssh_banner"]` or `["all"]`). Defaults to
  `["all"]`.
