...
Tests Cassandra/CQL server implementation using real `scylla` (ScyllaDB/Cassandra driver) client library. Validates connection startup, simple queries, prepared statements, error handling, and concurrent connections.
...
**Scripting Disabled**: All tests use `ServerConfig::new()` which disables scripting by default. Cassandra tests validate complex protocol flows (prepared statements, multiple queries) that benefit from action-based flexibility.

**Why no scripting?** Cassandra protocol has complex state (prepared statements, parameter binding) that is difficult to script. Action-based responses provide necessary flexibility.
...
cargo test --features e2e-tests,cassandra --test server::cassandra::e2e_test

# Run specific test
cargo test --features e2e-tests,cassandra --test server::cassandra::e2e_test test_cassandra_connection

# Run with output
cargo test --features e2e-tests,cassandra --test server::cassandra::e2e_test -- --nocapture
```
