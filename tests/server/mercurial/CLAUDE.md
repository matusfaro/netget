...
## Test Overview
Tests Mercurial HTTP server implementation with real HTTP clients (reqwest). Validates that Mercurial wire protocol commands work correctly over HTTP transport.
...
**Optimization potential**: Running tests in parallel could reduce to ~15-20s (longest test), but requires Ollama lock to prevent overload.
...
Tests are designed to validate protocol flow and response formats. Bundle generation testing is deferred to future work. This is acceptable for MVP - the important part is that the server responds correctly to the Mercurial HTTP wire protocol.
