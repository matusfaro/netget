...
Tests validate the Tor Directory server implementation by making HTTP requests to directory paths and verifying
LLM-generated consensus documents and microdescriptors.
...
**Consolidation**: Each test uses one server instance with comprehensive instructions, testing multiple aspects without
restarting.
...
**Current approach chosen for clarity**: Separate tests are easier to debug and understand expected behavior per
scenario.
