...
Tests IPP (Internet Printing Protocol) server using HTTP POST requests with binary IPP payloads. Validates that NetGet
can act as an IPP printer with LLM-controlled responses.
...
# Run IPP E2E tests
./cargo-isolated.sh test --features ipp --test server::ipp::test

# Run specific test
./cargo-isolated.sh test --features ipp --test server::ipp::test test_ipp_get_printer_attributes
```
