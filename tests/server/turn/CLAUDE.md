...
./cargo-isolated.sh test --features turn --test server::turn::e2e_test

# Run specific test
./cargo-isolated.sh test --features turn --test server::turn::e2e_test test_turn_basic_allocation

# Run with output
./cargo-isolated.sh test --features turn --test server::turn::e2e_test -- --nocapture

# Run with concurrency (uses Ollama lock)
./cargo-isolated.sh test --features turn --test server::turn::e2e_test -- --test-threads=4
```
