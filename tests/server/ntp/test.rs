...
async fn test_ntp_basic_query() -> E2EResult<()> {
    ...
    let config = NetGetConfig::new(prompt)
        ...
        .with_mock(|mock| {
            mock
                ...
                .expect_calls(1)
                .and()
                // Mock 2: NTP request received (rsntp may send multiple requests)
    ...
    // Wait for NTP server to fully initialize
    ...
    match client.synchronize(&address) {
        ...
        Err(e) => {
            ...
            match tokio::time::timeout(Duration::from_secs(5), socket.recv_from(&mut buffer)).await {
                ...
                Ok(Err(e)) => {
                    println!("  Raw NTP request failed: {}", e);
                }
                Err(_) => {
                    println!("  Raw NTP request timed out");
                }
            }
        }
    }
...
async fn test_ntp_stratum_levels() -> E2EResult<()> {
    ...
    match tokio::time::timeout(Duration::from_secs(5), socket.recv_from(&mut buffer)).await {
        ...
        Err(_) => {
            println!("Note: NTP stratum query timed out");
        }
    }
