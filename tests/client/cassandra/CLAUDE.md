...
E2E tests for the Cassandra client implementation. Tests verify client functionality using the NetGet binary in black-box mode.
...
**Instruction**:
- Server: "Listen on port {AVAILABLE_PORT} via Cassandra. Accept CQL queries. For SELECT * FROM system.local, return a result set with host_id and cluster_name columns."
