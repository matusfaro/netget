...
mod tests {
    ...
    fn test_dependency_name() {
        let dep = ProtocolDependency::SystemLibrary("pcap");
        assert_eq!(dep.name(), "libpcap");

        let dep = ProtocolDependency::ToolInPath("protoc");
        assert_eq!(dep.name(), "protoc");

        let dep = ProtocolDependency::PrivilegedPort(80);
        assert_eq!(dep.name(), "port 80");
    }

    #[test]
    fn test_dependency_description() {
        let dep = ProtocolDependency::SystemLibrary("pcap");
        assert!(dep.description().contains("libpcap"));

        let dep = ProtocolDependency::RawSocketAccess;
        assert!(dep.description().contains("CAP_NET_RAW"));
    }

    #[test]
    fn test_check_tool_in_path() {
        // 'sh' or 'cmd' should always be available
        #[cfg(unix)]
        assert!(check_tool_in_path("sh"));

        #[cfg(windows)]
        assert!(check_tool_in_path("cmd"));

        // Nonsense tool should not be available
        assert!(!check_tool_in_path("nonexistent_tool_xyz123"));
    }
}
