...
impl Protocol for BluetoothBleHeartRateProtocol {
    ...
    fn description(&self) -> &'static str { "BLE Heart Rate Service - monitor heart rate (BPM)" }
    fn example_prompt(&self) -> &'static str { "Act as a heart rate monitor. Start at 60 BPM, increase to 120 during exercise." }
...
impl Server for BluetoothBleHeartRateProtocol {
    fn spawn(&self, ctx: crate::protocol::SpawnContext) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<std::net::SocketAddr>> + Send>> {
        Box::pin(async move {
            ...
            let device_name = ctx.startup_params.as_ref().and_then(|p| p.get_optional_string("device_name")).as_deref().unwrap_or("NetGet-HeartRate").to_string();
    ...
    fn execute_action(&self, action: serde_json::Value) -> Result<ActionResult> {
        ...
        match action_type {
            ...
            _ => Err(anyhow::anyhow!("Unknown heart rate action: {}", action_type)),
        }
    }
}
