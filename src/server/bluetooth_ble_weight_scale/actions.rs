...
pub static WEIGHT_MEASUREMENT_EVENT: LazyLock<EventType> = LazyLock::new(|| {
    EventType::new("weight_measurement", "Weight measurement updated").with_parameters(vec![
        Parameter {
            ...
            description: "Weight in kilograms".to_string(),
            required: true,
        },
        Parameter {
            name: "bmi".to_string(),
...
impl Protocol for BluetoothBleWeightScaleProtocol {
        ...
        vec![
            ...
            ActionDefinition {
                ...
                parameters: vec![Parameter {
                    name: "bmi".to_string(),
                    type_hint: "number".to_string(),
                    description: "BMI value".to_string(),
                    required: true,
                }],
                example: json!({
                    "type": "set_bmi",
            ...
            ActionDefinition {
                ...
                parameters: vec![
                    Parameter {
                        ...
                        type_hint: "number".to_string(),
                        description: "User ID (1-9)".to_string(),
                        required: true,
                    },
                    Parameter {
                        name: "weight_kg".to_string(),
    ...
    fn description(&self) -> &'static str {
        "BLE Weight Scale - weight and BMI monitoring"
    }
    fn example_prompt(&self) -> &'static str {
        "Act as a weight scale showing 70.5 kg"
    }
...
impl Server for BluetoothBleWeightScaleProtocol {
    ...
    fn execute_action(&self, action: serde_json::Value) -> Result<ActionResult> {
        ...
        match action_type {
            "set_weight" | "set_bmi" | "multi_user" => Ok(ActionResult::Custom {
                name: action_type.to_string(),
                data: action,
            }),
            _ => Err(anyhow::anyhow!(
                "Unknown weight scale action: {}",
                action_type
            )),
        }
    }
