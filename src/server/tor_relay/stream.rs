...
mod tests {
    ...
    #[test]
    fn test_parse_begin_target() {
        let data = b"example.com:80\0";
        let target = parse_begin_target(data).unwrap();
        assert_eq!(target, "example.com:80");

        let data = b"192.168.1.1:443\0extra";
        let target = parse_begin_target(data).unwrap();
        assert_eq!(target, "192.168.1.1:443");
    }
    ...
    fn test_stream_manager() {
        ...
        let id1 = mgr.allocate_stream_id();
        let id2 = mgr.allocate_stream_id();

        assert_eq!(id1.as_u16(), 1);
        assert_eq!(id2.as_u16(), 2);

        mgr.create_stream(id1, "example.com:80".to_string()).unwrap();
        assert_eq!(mgr.count(), 1);

        let stream = mgr.get(id1).unwrap();
        assert_eq!(stream.target, "example.com:80");
    }

    #[test]
    fn test_build_relay_cell() {
        let cell = build_relay_cell(0x12345678, 42, relay_command::CONNECTED, b"test");
        assert_eq!(cell.len(), 514);
        assert_eq!(&cell[0..4], &[0x12, 0x34, 0x56, 0x78]); // Circuit ID
        assert_eq!(cell[4], 3); // RELAY command
        assert_eq!(cell[5], relay_command::CONNECTED); // Relay command
    }
}
