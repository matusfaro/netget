...
impl Http2Server {
    ...
    pub async fn spawn_with_llm_actions(
        ...
        info!("HTTP/2 server (action-based) listening on {}", local_addr);
...
async fn handle_http2_request_with_llm_actions(
    ...
    debug!(
        "HTTP/2 request (action-based): {} {} {} ({} bytes) from {:?}",
        method,
        uri,
        version,
        body_bytes.len(),
    ...
    trace!("HTTP/2 request headers:");
    for (name, value) in &headers {
        trace!("  {}: {}", name, value);
        let _ = status_tx.send(format!("[TRACE] HTTP/2 header: {}: {}", name, value));
    }
    if !body_bytes.is_empty() {
        if let Ok(body_str) = std::str::from_utf8(&body_bytes) {
            ...
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(body_str) {
                ...
                trace!("HTTP/2 request body (JSON):\n{}", pretty);
                let _ = status_tx.send(format!("[TRACE] HTTP/2 request body (JSON):\r\n{}", pretty.replace('\n', "\r\n")));
            } else {
                trace!("HTTP/2 request body:\n{}", body_str);
                let _ = status_tx.send(format!("[TRACE] HTTP/2 request body:\r\n{}", body_str.replace('\n', "\r\n")));
            }
        } else {
            trace!("HTTP/2 request body (binary): {} bytes", body_bytes.len());
            let _ = status_tx.send(format!("[TRACE] HTTP/2 request body (binary): {} bytes", body_bytes.len()));
        }
    }

    // Create HTTP/2 request event
    ...
    match call_llm(
        ...
        Ok(execution_result) => {
            ...
            // Extract HTTP/2 response from protocol results
