...
impl BluetoothBleRunningProtocol { pub fn new() -> Self { Self } }
...
impl Protocol for BluetoothBleRunningProtocol {
    ...
    fn protocol_name(&self) -> &'static str { "BLUETOOTH_BLE_RUNNING" }
    fn get_event_types(&self) -> Vec<EventType> { vec![RUNNING_MEASUREMENT_EVENT.clone()] }
    fn stack_name(&self) -> &'static str { "DATALINK" }
    fn keywords(&self) -> Vec<&'static str> { vec!["bluetooth", "ble", "running", "jogging", "fitness"] }
    ...
    fn description(&self) -> &'static str { "BLE Running - pace and cadence monitoring" }
    fn example_prompt(&self) -> &'static str { "Simulate running at 5:30 min/km pace with 180 SPM" }
