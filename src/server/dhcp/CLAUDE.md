...
## Overview
DHCP (Dynamic Host Configuration Protocol) server implementing RFC 2131 for automatic IP address assignment and network configuration. The LLM controls DHCP DISCOVER/OFFER/REQUEST/ACK flow using structured actions.
...
**Rationale**: dhcproto is the standard Rust library for DHCP. It handles the complex binary protocol (based on BOOTP) and provides type-safe DHCP option handling. The LLM doesn't need to understand DHCP packet structure - it just provides semantic data (IP, subnet mask, DNS servers) and the library handles encoding.
