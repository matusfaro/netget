...
## Overview
DNS (Domain Name System) server implementing RFC 1035 standard for domain name resolution. The LLM can respond to queries with A, AAAA, CNAME, MX, TXT records, and NXDOMAIN responses using structured actions.
...
**Rationale**: hickory-proto is the de facto standard for DNS in Rust. It provides robust parsing/serialization and eliminates the need for manual binary protocol handling. The LLM doesn't need to understand DNS binary format - it just provides semantic data (domain, IP, TTL) and the library handles encoding.
