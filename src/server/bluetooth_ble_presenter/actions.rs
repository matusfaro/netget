...
impl Protocol for BluetoothBlePresenterProtocol {
    ...
    fn metadata(&self) -> crate::protocol::metadata::ProtocolMetadataV2 {
        ...
        ProtocolMetadataV2::builder().state(DevelopmentState::Experimental)
            .implementation("BLE Presenter").llm_control("Presenter actions")
            .e2e_testing("Requires BLE device").notes("BLE Presenter").build()
    }
    fn description(&self) -> &'static str { "BLE Presenter" }
    fn example_prompt(&self) -> &'static str { "Act as a BLE presenter device" }
    ...
    fn spawn(&self, ctx: crate::protocol::SpawnContext) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<std::net::SocketAddr>> + Send>> {
        Box::pin(async move {
            crate::server::bluetooth_ble_presenter::BluetoothBlePresenter::spawn_with_llm_actions(
                "NetGet-Presenter".to_string(), ctx.llm_client, ctx.app_state, ctx.status_tx, ctx.server_id, ctx.instruction
            ).await
        })
    }
