...
## Overview
DNS-over-TLS server implementing RFC 7858 for secure DNS queries over TLS transport. The LLM controls DNS responses while NetGet handles TLS encryption and connection management.
...
**Rationale**: DoT is essentially DNS delivered over TLS instead of plain UDP/TCP. By combining hickory-proto's DNS handling with tokio-rustls's TLS transport, we get a complete secure DNS solution. The LLM can focus on DNS semantics while the libraries handle encryption.
