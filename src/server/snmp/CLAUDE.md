...
## Overview
SNMP agent implementing RFC 1157 (SNMPv1) and RFC 3416 (SNMPv2c) using the rasn-snmp library. Provides network management functionality where the LLM controls OID responses (system info, interface stats, custom MIBs). Uses UDP transport for request/response protocol.
...
**Rationale**: rasn-snmp is the most mature pure-Rust SNMP library with async support. However, it focuses on parsing (client-side), not response building (server-side). We use it for parsing requests and then manually encode responses using BER. This gives us full control over response format and allows the LLM to return simple JSON which we convert to proper SNMP packets.
...
**Rationale**: v1/v2c cover 90% of SNMP deployments. v3 adds significant complexity (encryption, user management, key exchange).
