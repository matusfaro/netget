...
pub static ELASTICSEARCH_REQUEST_EVENT: LazyLock<EventType> = LazyLock::new(|| {
    ...
    .with_parameters(vec![
        ...
        Parameter {
            ...
            type_hint: "string".to_string(),
            description: "Detected operation type (search, index, get, delete, bulk, etc.)".to_string(),
            required: true,
        },
        Parameter {
    ...
    ActionDefinition {
        ...
        parameters: vec![
            Parameter {
                ...
                description: "Array of search result documents with _id, _index, _source".to_string(),
                required: true,
            },
            Parameter {
...
pub fn get_elasticsearch_event_types() -> Vec<EventType> {
    vec![
        ELASTICSEARCH_REQUEST_EVENT.clone(),
    ]
}
    ...
    fn execute_action(&self, action: Value) -> Result<ActionResult> {
        ...
        match action_type {
            ...
            "send_search_response" => {
                let hits = action.get("hits")
                    .ok_or_else(|| anyhow::anyhow!("Missing hits"))?;

                let total = action.get("total")
                    .and_then(|v| v.as_u64())
                    .unwrap_or_else(|| {
                        hits.as_array().map(|a| a.len() as u64).unwrap_or(0)
                    });

                let took = action.get("took")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(10);
            ...
            "send_bulk_response" => {
                let items = action.get("items")
                    .ok_or_else(|| anyhow::anyhow!("Missing items"))?;

                let errors = action.get("errors")
                    .and_then(|v| v.as_bool())
                    .unwrap_or(false);
            ...
            "send_cluster_info" => {
                let cluster_name = action.get("cluster_name")
                    .and_then(|v| v.as_str())
                    .unwrap_or("llm-elasticsearch");

                let _status = action.get("status")
                    .and_then(|v| v.as_str())
                    .unwrap_or("green");

                let version = action.get("version")
                    .and_then(|v| v.as_str())
                    .unwrap_or("8.0.0-llm");
