...
impl Server for BitcoinProtocol {
    ...
    fn get_startup_parameters(&self) -> Vec<ParameterDefinition> {
        vec![ParameterDefinition {
            ...
            description: "Bitcoin network: 'mainnet', 'testnet', 'signet', or 'regtest' (default: mainnet)".to_string(),
            required: false,
...
impl BitcoinProtocol {
    ...
    fn execute_send_version(&self, action: serde_json::Value) -> Result<ActionResult> {
        ...
        let receiver = Address::new(&std::net::SocketAddr::from(([0, 0, 0, 0], 0)), ServiceFlags::NONE);
        let sender = Address::new(&std::net::SocketAddr::from(([0, 0, 0, 0], 0)), ServiceFlags::from(services));
