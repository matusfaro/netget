...
impl BluetoothBleFileTransfer {
    #[cfg(feature = "bluetooth-ble-file_transfer")]
    pub async fn spawn_with_llm_actions(_: String, llm: crate::llm::ollama_client::OllamaClient, state: Arc<crate::state::app_state::AppState>, tx: mpsc::UnboundedSender<String>, id: crate::state::ServerId, inst: String) -> Result<std::net::SocketAddr> {
        crate::server::bluetooth_ble::BluetoothBle::spawn_with_llm_actions("NetGet-FileTransfer".to_string(), llm, state, tx, id, format!("{}. Configure as BLE File Transfer.", inst)).await
    }
}
#[cfg(not(feature = "bluetooth-ble-file_transfer"))]
impl BluetoothBleFileTransfer {
