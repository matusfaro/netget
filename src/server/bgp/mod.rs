                    ...
                    Ok((stream, remote_addr)) => {
                        ...
                        tokio::spawn(async move {
                            ...
                            if let Err(e) = session.handle().await {
                                ...
                                let _ = status_clone.send(format!("[ERROR] BGP session error: {}", e));
                            }

                            info!("BGP connection {} closed", connection_id);
                            let _ = status_clone.send(format!("✗ BGP connection {} closed", connection_id));
                        });
                    }
                    Err(e) => {
...
impl BgpSession {
    ...
    async fn handle_keepalive_message(&mut self) -> Result<()> {
        ...
        match self.session_state {
            ...
            _ => {
                warn!("BGP KEEPALIVE in unexpected state: {:?}", self.session_state);
            }
        }
    ...
    async fn send_open_message(&mut self) -> Result<()> {
        ...
        let router_id_parts: Vec<u8> = self.router_id
            .split('.')
    ...
    async fn send_notification(&mut self, error_code: u8, error_subcode: u8, data: &[u8]) -> Result<()> {
        let mut msg = Vec::new();
