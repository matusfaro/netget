...
impl BluetoothBleEnvironmentalProtocol { pub fn new() -> Self { Self } }
...
impl Protocol for BluetoothBleEnvironmentalProtocol {
    ...
    fn protocol_name(&self) -> &'static str { "BLUETOOTH_BLE_ENVIRONMENTAL" }
    fn get_event_types(&self) -> Vec<EventType> { vec![] }
    fn stack_name(&self) -> &'static str { "DATALINK" }
    fn keywords(&self) -> Vec<&'static str> { vec!["bluetooth", "ble", "environmental"] }
    fn metadata(&self) -> crate::protocol::metadata::ProtocolMetadataV2 {
        use crate::protocol::metadata::{ProtocolMetadataV2, DevelopmentState};
        ProtocolMetadataV2::builder().state(DevelopmentState::Experimental)
            .implementation("BLE Environmental").llm_control("Environmental actions")
            .e2e_testing("Requires BLE device").notes("BLE Environmental").build()
    }
    fn description(&self) -> &'static str { "BLE Environmental" }
    fn example_prompt(&self) -> &'static str { "Act as a BLE environmental device" }
    ...
    fn spawn(&self, ctx: crate::protocol::SpawnContext) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<std::net::SocketAddr>> + Send>> {
        Box::pin(async move {
            crate::server::bluetooth_ble_environmental::BluetoothBleEnvironmental::spawn_with_llm_actions(
                "NetGet-Environmental".to_string(), ctx.llm_client, ctx.app_state, ctx.status_tx, ctx.server_id, ctx.instruction
            ).await
        })
    }
