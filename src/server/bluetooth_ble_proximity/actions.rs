...
impl BluetoothBleProximityProtocol { pub fn new() -> Self { Self } }
...
impl Protocol for BluetoothBleProximityProtocol {
    ...
    fn protocol_name(&self) -> &'static str { "BLUETOOTH_BLE_PROXIMITY" }
    fn get_event_types(&self) -> Vec<EventType> { vec![] }
    fn stack_name(&self) -> &'static str { "DATALINK" }
    fn keywords(&self) -> Vec<&'static str> { vec!["bluetooth", "ble", "proximity"] }
    fn metadata(&self) -> crate::protocol::metadata::ProtocolMetadataV2 {
        use crate::protocol::metadata::{ProtocolMetadataV2, DevelopmentState};
        ProtocolMetadataV2::builder().state(DevelopmentState::Experimental)
            .implementation("BLE Proximity").llm_control("Proximity actions")
            .e2e_testing("Requires BLE device").notes("BLE Proximity").build()
    }
    fn description(&self) -> &'static str { "BLE Proximity" }
    fn example_prompt(&self) -> &'static str { "Act as a BLE proximity device" }
