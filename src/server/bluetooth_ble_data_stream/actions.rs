...
pub static STREAM_STARTED_EVENT: LazyLock<EventType> = LazyLock::new(|| {
    EventType::new("stream_started", "Data stream started", json!({"type": "placeholder", "event_id": "stream_started"})).with_parameters(vec![
        Parameter {
            ...
            description: "Stream identifier".to_string(),
            required: true,
        },
        Parameter {
            name: "sample_rate".to_string(),
            type_hint: "number".to_string(),
            description: "Samples per second".to_string(),
            required: true,
        },
    ])
    .with_log_template(
...
pub static STREAM_DATA_EVENT: LazyLock<EventType> = LazyLock::new(|| {
    EventType::new("stream_data", "Stream data packet received", json!({"type": "placeholder", "event_id": "stream_data"})).with_parameters(vec![
        ...
        Parameter {
            ...
            type_hint: "number".to_string(),
            description: "Packet sequence number".to_string(),
            required: true,
        },
    ])
    .with_log_template(
...
impl Protocol for BluetoothBleDataStreamProtocol {
            ...
            ActionDefinition {
                ...
                parameters: vec![
                    ...
                    Parameter {
                        name: "sample_rate".to_string(),
                        type_hint: "number".to_string(),
                        description: "Samples per second (1-100)".to_string(),
                        required: true,
                    },
                    Parameter {
                        name: "data_type".to_string(),
                        type_hint: "string".to_string(),
                        description: "Data type (sensor, gps, imu, audio)".to_string(),
                        required: true,
                    },
                ],
                example: json!({
            ...
            ActionDefinition {
                ...
                parameters: vec![
                    ...
                    Parameter {
                        ...
                        description: "Data payload (JSON)".to_string(),
                        required: true,
                    },
                ],
                example: json!({
    ...
    fn description(&self) -> &'static str {
        "BLE data streaming - real-time sensor/telemetry data"
    }
    fn example_prompt(&self) -> &'static str {
        "Stream IMU data at 10 Hz with random accelerometer values"
    }
...
impl Server for BluetoothBleDataStreamProtocol {
    ...
    fn execute_action(&self, action: serde_json::Value) -> Result<ActionResult> {
        ...
        match action_type {
            "start_stream" | "send_stream_data" | "stop_stream" => Ok(ActionResult::Custom {
                name: action_type.to_string(),
                data: action,
            }),
            _ => Err(anyhow::anyhow!(
                "Unknown data stream action: {}",
                action_type
            )),
        }
    }
