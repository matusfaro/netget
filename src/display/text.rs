...
impl TextRenderer {
    ...
    pub fn draw_text(&mut self, pixmap: &mut Pixmap, x: u32, y: u32, text: &str, font_size: u32, color: Color) {
        let mut buffer = Buffer::new(&mut self.font_system, Metrics::new(font_size as f32, font_size as f32));
                    ...
                    for (img_y, row) in image.data.chunks_exact(image.placement.width as usize).enumerate() {
                        for (img_x, &alpha) in row.iter().enumerate() {
                            ...
                            if px >= 0 && py >= 0 && px < pixmap.width() as i32 && py < pixmap.height() as i32 {
                                ...
                                let r = (color.r as f32 * alpha_f + existing.red() as f32 * (1.0 - alpha_f)) as u8;
                                let g = (color.g as f32 * alpha_f + existing.green() as f32 * (1.0 - alpha_f)) as u8;
                                let b = (color.b as f32 * alpha_f + existing.blue() as f32 * (1.0 - alpha_f)) as u8;
