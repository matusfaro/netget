...
pub async fn run_tui(
    ...
    app.add_message("Example prompts:".to_string());
    app.add_message("  listen on port 2121 and pretend to be FTP server".to_string());
    app.add_message("  listen on port 8080 and pretend to be HTTP server".to_string());
    app.add_message("".to_string());
...
async fn handle_network_event(
    ...
    match event {
        ...
        NetworkEvent::Connected { connection_id, remote_addr } => {
            ...
            tokio::spawn(async move {
                handle_connection_greeting(connection_id, &state, &connections, &connection_states, &status_tx).await;
            });
        }
    ...
    let _has_queued = {
        ...
        if let Some(conn_state) = states.get_mut(&connection_id) {
            if !conn_state.queue.is_empty() {
                let _queued_data = bytes::Bytes::copy_from_slice(&conn_state.queue);
                conn_state.queue.clear();

                // Don't spawn recursively - just note that we need to process it
                // The caller will handle the next iteration
                true
            } else {
                conn_state.status = ConnectionStatus::Idle;
                false
...
async fn execute_action_background(
    ...
    match action {
        Action::UpdateInstruction { instruction } => {
            ...
            let _ = status_tx.send("Instruction updated".to_string());
        }
