...
mod tests {
    ...
    #[test]
    fn test_parse_theme() {
        assert_eq!(parse_theme("auto").unwrap(), None);
        assert_eq!(parse_theme("AUTO").unwrap(), None);
        assert_eq!(parse_theme("light").unwrap(), Some(Theme::Light));
        assert_eq!(parse_theme("dark").unwrap(), Some(Theme::Dark));
        assert_eq!(parse_theme("neutral").unwrap(), Some(Theme::Neutral));
    ...
    fn test_color_palette_creation() {
        ...
        assert_eq!(light.error, Color::DarkRed);
        assert_eq!(light.warning, Color::DarkYellow);

        let neutral = ColorPalette::neutral();
        assert_eq!(neutral.error, Color::Red);
        assert_eq!(neutral.warning, Color::DarkYellow);
    }

    #[test]
    fn test_from_theme() {
        let dark = ColorPalette::from_theme(Theme::Dark);
        assert_eq!(dark.error, Color::Red);

        let light = ColorPalette::from_theme(Theme::Light);
        assert_eq!(light.error, Color::DarkRed);

        let neutral = ColorPalette::from_theme(Theme::Neutral);
        assert_eq!(neutral.error, Color::Red);
    }
}
