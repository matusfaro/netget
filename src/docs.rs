...
pub fn list_all_protocols() -> String {
    ...
    // NetGet description
    output.push_str(&format!(
        "{}NetGet is an experimental network application where an LLM (via Ollama){}\n",
        colors::DIM,
        colors::RESET
    ));
    output.push_str(&format!(
        "{}controls network protocols and acts as a server for 50+ protocols.{}\n",
        colors::DIM,
        colors::RESET
    ));
    ...
    output.push_str(&format!("  {}•{} {}Scripting:{} LLM generates on-the-fly Python/JavaScript code to reduce LLM calls\n",
        ...
        colors::BOLD, colors::RESET));
    output.push_str(&format!(
        "  {}•{} {}Web Search:{} LLM can fetch protocol RFCs and documentation from the web\n",
        colors::GREEN,
        colors::RESET,
        colors::BOLD,
        colors::RESET
    ...
    output.push_str(&format!(
        "  {}•{} {}File Reading:{} LLM can read local files (schemas, configs, prompts)\n",
        colors::GREEN,
        colors::RESET,
        colors::BOLD,
        colors::RESET
    ...
    output.push_str(&format!(
        "  {}•{} {}Logging:{} Comprehensive logging system (TRACE/DEBUG/INFO/WARN/ERROR levels)\n",
        colors::GREEN,
        colors::RESET,
        colors::BOLD,
        colors::RESET
    ...
    output.push_str(&format!(
        "  {}•{} {}Action-Based:{} Structured JSON responses for precise protocol control\n",
        colors::GREEN,
        colors::RESET,
        colors::BOLD,
        colors::RESET
    ...
    output.push_str(&format!(
        "{}Use{} {}{}/docs <protocol>{} {}to see detailed information.{}\n\n",
        colors::DIM,
        colors::RESET,
        colors::CYAN,
        colors::BOLD,
    ...
    let mut groups: std::collections::HashMap<&'static str, Vec<(String, DevelopmentState)>> =
    ...
    for (protocol_name, protocol) in &all_protocols {
        ...
        groups
            .entry(group)
            .or_default()
            .push((protocol_name.clone(), state));
    }
    ...
    for group_name in sorted_group_names {
        if let Some(protocols) = groups.get(group_name) {
            ...
            let colored_protocols: Vec<String> = sorted_protocols
                .iter()
                .map(|(name, state)| {
            ...
            let protocol_list =
                colored_protocols.join(&format!("{}, {}", colors::RESET, colors::DIM));
...
pub fn show_protocol_docs(protocol_name: &str) -> Result<String, String> {
    ...
    let parsed_protocol_name = registry.parse_from_str(protocol_name).ok_or_else(|| {
        format!(
            "{}Unknown protocol: {}{}. Use /docs to see all protocols.",
            colors::RED,
            protocol_name,
            colors::RESET
        )
    ...
    let protocol = registry.get(&parsed_protocol_name).ok_or_else(|| {
        format!(
            "{}Protocol {} not found in registry{}",
            colors::RED,
            parsed_protocol_name,
            colors::RESET
        )
    })?;

    let metadata = protocol.metadata();
    let stack_name = registry
        .stack_name_by_protocol(&parsed_protocol_name)
        .unwrap_or("UNKNOWN");
    ...
    if let Some(protocol) = registry.get(&parsed_protocol_name) {
        ...
        if !params.is_empty() {
            ...
            output.push_str(&format!(
                "{}These parameters can be provided when opening the server:{}\n\n",
                colors::DIM,
                colors::RESET
            ));
            for param in params {
        ...
        if !event_types.is_empty() {
            ...
            output.push_str(&format!(
                "{}This protocol can emit the following network events:{}\n\n",
                colors::DIM,
                colors::RESET
            ));
            ...
            for event_type in event_types {
                ...
                if !event_type.actions.is_empty() {
                    ...
                    output.push_str(&format!(
                        "  {}No specific actions available for this event.{}\n\n",
                        colors::GREY,
                        colors::RESET
                    ));
                }
            ...
            output.push_str(&format!(
                "{}This protocol hasn't documented its event types yet.{}\n\n",
                colors::GREY,
                colors::RESET
            ));
        }
