...
impl Client for KafkaClientProtocol {
    ...
    fn get_sync_actions(&self) -> Vec<ActionDefinition> {
        vec![
            ActionDefinition {
                name: "produce_message".to_string(),
                description: "Produce a message in response to received data (producer mode)".to_string(),
                parameters: vec![
                    Parameter {
                        name: "topic".to_string(),
                        type_hint: "string".to_string(),
    ...
    fn execute_action(&self, action: serde_json::Value) -> Result<ClientActionResult> {
        ...
        match action_type {
            ...
            _ => Err(anyhow::anyhow!("Unknown Kafka client action: {}", action_type)),
        }
    }
    ...
    fn keywords(&self) -> Vec<&'static str> {
        vec!["kafka", "kafka client", "connect to kafka", "kafka producer", "kafka consumer"]
    }
