            ...
            match result {
                        ...
                        "ldap_modify" => {
                            ...
                            let modify_result = tokio::task::spawn_blocking(move || {
                                ...
                                let mod_op = match operation_owned.as_str() {
                                    "add" => {
                                        Mod::Add(attribute_owned.as_str(), values_refs.clone())
                                    }
                                    "delete" => {
                                        Mod::Delete(attribute_owned.as_str(), values_refs.clone())
                                    }
                                    "replace" => {
                                        Mod::Replace(attribute_owned.as_str(), values_refs)
                                    }
                                    _ => {
                ...
                ClientActionResult::Disconnect => {
                    ...
                    app_state
                        ...
                        .await;
                    let _ =
                        status_tx.send(format!("[CLIENT] LDAP client {} disconnected", client_id));
                    let _ = status_tx.send("__UPDATE_UI__".to_string());
