...
## Overview
LDAP (Lightweight Directory Access Protocol) client for connecting to LDAP directory servers and performing directory operations. Supports bind (authentication), search, add, modify, and delete operations with full LLM control.
...
- **Blocking operations** - All LDAP operations (bind, search, add, modify, delete) use spawn_blocking to avoid blocking async runtime
