...
The PyPI (Python Package Index) client protocol implementation provides LLM-controlled interaction with the Python Package Index for searching, downloading, and querying package information.
...
### Connection Model
PyPI client is "connectionless" - no persistent TCP connection is maintained. Each operation makes independent HTTPS requests to the PyPI JSON API.
