...
## Overview
SNMP client implementing RFC 1157 (SNMPv1) and RFC 3416 (SNMPv2c) using the rasn-snmp library. Enables LLM-controlled SNMP operations against network devices and agents - querying OIDs, walking MIB trees, and modifying agent values. Uses UDP transport for request/response protocol.
...
**Rationale**: rasn-snmp provides clean API for both encoding requests and decoding responses. Unlike the server (which uses manual BER encoding for responses), the client can use rasn-snmp's built-in encoding for all request types. This simplifies request construction while maintaining full protocol compliance.
