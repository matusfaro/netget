...
impl Protocol for WhoisClientProtocol {
                ...
                ActionDefinition {
                    ...
                    parameters: vec![
                        Parameter {
                            name: "query".to_string(),
                            type_hint: "string".to_string(),
                            description: "Domain name or IP address to query (e.g., 'example.com' or '8.8.8.8')".to_string(),
                            required: true,
                        },
                    ],
                    example: json!({
        ...
        fn get_event_types(&self) -> Vec<EventType> {
            vec![
                EventType {
                    id: "whois_connected".to_string(),
                    description: "Triggered when WHOIS client connects to server".to_string(),
                    actions: vec![],
                    parameters: vec![],
                },
                EventType {
                    id: "whois_response_received".to_string(),
                    description: "Triggered when WHOIS client receives a response".to_string(),
                    actions: vec![],
                    parameters: vec![],
                },
            ]
...
impl Client for WhoisClientProtocol {
        ...
        fn execute_action(&self, action: serde_json::Value) -> Result<ClientActionResult> {
            ...
            match action_type {
                ...
                _ => Err(anyhow::anyhow!("Unknown WHOIS client action: {}", action_type)),
            }
        }
}
