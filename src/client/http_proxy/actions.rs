...
impl Client for HttpProxyClientProtocol {
        ...
        match action_type {
            "establish_tunnel" => {
                ...
                let target_port = action
                    .get("target_port")
                    .and_then(|v| v.as_u64())
                    .context("Missing 'target_port' field")? as u16;
            ...
            _ => Err(anyhow::anyhow!("Unknown HTTP proxy client action: {}", action_type)),
        }
    }
    ...
    fn get_event_types(&self) -> Vec<EventType> {
        vec![
            EventType {
                ...
                description: "Triggered when HTTP proxy client connects to proxy server".to_string(),
                actions: vec![],
                parameters: vec![],
            },
            EventType {
    ...
    fn keywords(&self) -> Vec<&'static str> {
        vec!["http proxy", "http proxy client", "connect via proxy", "proxy"]
    }
