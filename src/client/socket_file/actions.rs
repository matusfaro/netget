...
impl Protocol for SocketFileClientProtocol {
        ...
        fn get_event_types(&self) -> Vec<EventType> {
            vec![
                EventType {
                    ...
                    description: "Triggered when Socket File client connects to Unix domain socket".to_string(),
                    actions: vec![],
                    parameters: vec![],
                },
                EventType {
                    id: "socket_file_data_received".to_string(),
                    description: "Triggered when Socket File client receives data from Unix domain socket".to_string(),
                    actions: vec![],
                    parameters: vec![],
                },
            ]
        ...
        fn keywords(&self) -> Vec<&'static str> {
            vec!["socket file", "unix socket", "domain socket", "socket-file", "socketfile"]
        }
...
impl Client for SocketFileClientProtocol {
        ...
        fn execute_action(&self, action: serde_json::Value) -> Result<ClientActionResult> {
            ...
            match action_type {
                ...
                _ => Err(anyhow::anyhow!("Unknown Socket File client action: {}", action_type)),
            }
        }
}
