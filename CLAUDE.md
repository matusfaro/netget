...
./cargo-isolated.sh build --no-default-features --features tcp 2>&1 | tee build.log

# Test and save output
./cargo-isolated.sh test --no-default-features --features tcp 2>&1 | tee test.log
```
...
grep "error\[E0425\]" build.log  # Unresolved names
grep "error\[E0599\]" build.log  # Method not found
...
./cargo-isolated.sh build 2>&1 | tee build.log

# Analyze ALL errors
grep "error\[E" build.log  # Shows all 15 errors
