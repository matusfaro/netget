...
**Command:**
```bash
./cargo-isolated.sh test --features <protocol> --test server -- --test-threads=1 --nocapture > test_results.log 2>&1
```
**Duration:** 1-3 hours
...
./cargo-isolated.sh test --features <protocol> --test server -- --test-threads=1 --nocapture
```

### Run Specific Protocol
```bash
# SSH
./cargo-isolated.sh test --features <protocol> --test server ssh -- --nocapture

# HTTP
./cargo-isolated.sh test --features <protocol> --test server http -- --nocapture

# Database protocol (example: MySQL)
./cargo-isolated.sh test --features <protocol>,mysql --test server mysql -- --nocapture
```

### Run Without Privileged Ports
```bash
# Skip IPSec, DHCP, and other privileged port tests
./cargo-isolated.sh test --features <protocol> --test server -- --test-threads=1 --skip ipsec --skip dhcp
```

### Save Results
```bash
# Capture all output
./cargo-isolated.sh test --features <protocol> --test server -- --test-threads=1 --nocapture | tee test_results_$(date +%Y%m%d_%H%M%S).log
```
